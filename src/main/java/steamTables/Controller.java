...
public class Controller {
    ...
    public Steam findTheSteamUsingTV(double T, double v) { // it's working
        ...
        if (T >= steam.getCriticalTemperature() || T > 373.95) { // superheated
            ...
            if (!found) {
                return interpolatedSuperHeatedOrCompressed(T,v,"T","V",SuperH);
            }
        }
        else {
            ...
            if (g){
                ...
                return interpolatedSuperHeatedOrCompressed(T,v,"T","V",table);
                }
            }
        return steam;
    }
    ...
    public Steam findTheSteamUsingTH(double T, double h) {
        ...
        if (T >= steam.getCriticalTemperature() || T > 373.95) { // superheated
            ...
            if (!found) {
                return interpolatedSuperHeatedOrCompressed(T,h,"T","H",SuperH);
            }
        }
        else {
            double [][] saturated = db.getSaturatedTableT();
            ...
            if (g){
            ...
            else if (saturated[row][7] < h && (saturated[row][8] > h)) {
                ...
                return interpolatedSuperHeatedOrCompressed(T,h,"T","H",table);
            }
        }
        return steam;
    }
        ...
        if (f){
            ...
            for (int i = 0; i < table.length; i++) {
                if (P == table[i][0]) {
                    ...
                    if (i!= table.length-1 &&table[i][2] < v && table[i + 1][2] > v) { // interpolation
                        double interpolatedT = Interpolation.linear(table[i][3], table[i][1], table[i + 1][3], table[i + 1][1], v);
                        double interpolatedU = Interpolation.linear(table[i][3], table[i][2], table[i + 1][3], table[i + 1][3], v);
                        double interpolatedH = Interpolation.linear(table[i][3], table[i][4], table[i + 1][3], table[i + 1][4], v);
                        double interpolatedS = Interpolation.linear(table[i][3], table[i][5], table[i + 1][3], table[i + 1][5], v);
